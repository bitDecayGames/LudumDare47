...
class PlayState3 extends FlxState {
	...
	var defaultBpm = 135.0; // hardcode these for now, but we could ideally get them from FMOD (but not for this jam)
	...
	private function Rewind() {
		Timer.delay(() -> {
			...
			var fmodRewind3 = FmodManager.PlaySoundWithReference(FmodSFX.Rewind);
			trace("Playstate3: Rewind sound ID: " + fmodRewind3);
			level.groundSpeed = 0;
			// cancel any in-progress tweens
			for (t in tweens) {
				if (!t.finished) {
			...
			FmodManager.RegisterCallbacksForSound(fmodRewind3, () -> {
				level.rewind = true;
				...
				FmodManager.RegisterCallbacksForSound(fmodRewind3, () -> {
					...
					FmodFlxUtilities.TransitionToState(new PlayState3());
